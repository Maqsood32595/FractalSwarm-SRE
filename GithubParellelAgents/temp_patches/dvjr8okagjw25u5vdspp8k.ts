...
type Microseconds = 'microseconds' | 'microsecond' | 'microsecs' | 'microsec' | 'us' | 'μs';
type Unit =
  | Years
  | Months
  | Weeks
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>milliseconds?|msecs?|ms|microseconds?|microsecs?|us|µs|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  if (msAbs >= us) { // Handle microseconds
