...
const y = d * 365.25;
const mo = y / 12;
const us = 1 / 1000; // Microseconds in milliseconds
...
type Microseconds = 'microseconds' | 'microsecond' | 'us' | 'μs'; // Added microseconds
type Unit =
  | Years
  | Months
  | Weeks
...
export function parse(str: string): number {
  ...
  switch (matchUnit) {
    ...
    case 'ms':
      return n;
    case 'microseconds': // Added microsecond handling
    case 'microsecond':
    case 'us':
    case 'μs':
      return n * us;
...
function fmtShort(ms: number): StringValue {
  ...
  // Only format as microseconds if the value is less than 1 millisecond but at least a microsecond.
  if (msAbs < 1 && msAbs >= us) {
    return `${Math.round(ms / us)}us`;
  }
  return `${ms}ms`;
}
