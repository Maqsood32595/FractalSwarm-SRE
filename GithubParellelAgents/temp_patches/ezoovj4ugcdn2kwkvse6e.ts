...
function plural(
  ms: number,
  msAbs: number, // Keep this parameter as fmtLong still passes it, but it's not directly used for the pluralization check
  n: number,
  name: string,
): StringValue {
  const count = Math.round(ms / n);
  const isPlural = Math.abs(count) !== 1;
  return `${count} ${name}${isPlural ? 's' : ''}` as StringValue;
}