...
function plural(
  ...
  // Bug fix: Math.round with negative numbers rounds towards zero for halves.
  // To ensure consistent rounding for magnitude, round the absolute value and then apply the sign.
  return `${Math.round(msAbs / n) * Math.sign(ms)} ${name}${isPlural ? 's' : ''}` as StringValue;
}