...
const y = d * 365.25;
const mo = y / 12;
const us = 0.001; // Define microseconds constant correctly: 1 microsecond = 0.001 milliseconds
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  // Handle microseconds in short format
  if (msAbs >= us) {
...
function fmtLong(ms: number): StringValue {
  ...
  // Handle microseconds in long format
  if (msAbs >= us) {
