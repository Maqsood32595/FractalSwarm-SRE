...
const y = d * 365.25;
const mo = y / 12;
const us = 1 / 1000; // microsecond constant (0.001 milliseconds)
...
type Microseconds = 'microseconds' | 'microsecond' | 'μs' | 'us'; // New microsecond type
type Unit =
  | Years
  | Months
  | Weeks
  ...
  | Seconds
  | Milliseconds
  | Microseconds; // Add Microseconds to Unit type
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>milliseconds?|msecs?|ms|microseconds?|microsecond?|μs|us|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
  ...
  switch (matchUnit) {
    ...
    case 'ms':
      return n;
    case 'microseconds': // Handle microseconds
    case 'microsecond':
    case 'μs':
    case 'us':
      return n * us; // n microseconds * (0.001 ms/microsecond) = milliseconds
    default:
...
function fmtShort(ms: number): StringValue {
  ...
  // If we reach here, msAbs is less than 1 second.
  // We now handle microseconds, then fall back to '0ms' for values that round to 0us.
  const roundedMicroseconds = Math.round(ms / us);
  if (roundedMicroseconds !== 0) {
      return `${roundedMicroseconds}us`;
  }

  // If roundedMicroseconds is 0, it means the original `ms` value was either 0
  // or very small and rounded to 0 microseconds (e.g., 0.4us).
  // In this case, we return '0ms'.
  return '0ms';
}
...
function fmtLong(ms: number): StringValue {
  ...
  // If we reach here, msAbs is less than 1 second.
  // We now handle microseconds, then fall back to '0 ms' for values that round to 0us.
  const roundedMicroseconds = Math.round(ms / us);
  if (roundedMicroseconds !== 0) {
      return plural(ms, msAbs, us, 'microsecond');
  }

  // If roundedMicroseconds is 0, it means the original `ms` value was either 0
  // or very small and rounded to 0 microseconds (e.g., 0.4us).
  // In this case, we return '0 ms'.
  return '0 ms';
}
