const us = 1 / 1000; // 1 microsecond = 0.001 milliseconds
const s = 1000;
const m = s * 60;
const h = m * 60;
const d = h * 24;
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|microsecs?|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  if (msAbs < 1 && msAbs > 0) { // If it's less than a millisecond but greater than 0, treat as microseconds
    return `${Math.round(ms / us)}μs`;
  }
  return `${ms}ms`;
}
