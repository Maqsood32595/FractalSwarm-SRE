...
type Microseconds = 'microseconds' | 'microsecond' | 'usecs' | 'usec' | 'us' | 'μs';
type Unit =
  | Years
  | Months
  | Weeks
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|microsecond|usecs?|usec|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
