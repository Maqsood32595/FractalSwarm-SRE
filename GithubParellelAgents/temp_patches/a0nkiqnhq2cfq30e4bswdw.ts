...
type Unit =
  ...
  | Seconds
  | Milliseconds
  | Microseconds; // Added Microseconds
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|usecs?|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
  ...
  switch (matchUnit) {
    ...
    case 'us':
    case 'μs':
      return n / 1000; // 1 microsecond = 0.001 milliseconds
    default:
...
function fmtShort(ms: number): StringValue {
  ...
  // Added microsecond formatting if ms is less than 1
  if (msAbs < 1 && msAbs > 0) {
...
function fmtLong(ms: number): StringValue {
  ...
  // Added microsecond formatting if ms is less than 1
  if (msAbs < 1 && msAbs > 0) {
    return plural(ms * 1000, msAbs * 1000, 1, 'microsecond');
  }
  return `${ms} ms`;
}
