...
type Microseconds = 'microseconds' | 'microsecond' | 'us' | 'μs';
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?(?:\d+(?:\.\d*)?|\.\d+))\s*(?<unit>microseconds?|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
