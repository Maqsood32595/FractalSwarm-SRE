...
const y = d * 365.25;
const mo = y / 12;
const us = s / 1000; // Microseconds constant
...
type Microseconds = 'microseconds' | 'microsecond' | 'usecs' | 'usec' | 'us' | 'μs'; // New Microseconds type
type Unit =
  | Years
  | Months
  | Weeks
  ...
  | Seconds
  | Milliseconds
  | Microseconds; // Add Microseconds to Unit
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|usecs?|usec|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec( // Updated regex to include microseconds
      str,
    );
  ...
  switch (matchUnit) {
    ...
    case 'ms':
      return n;
    case 'microseconds': // New case for microseconds
    case 'microsecond':
    case 'usecs':
    case 'usec':
    case 'us':
...
function fmtShort(ms: number): StringValue {
  ...
  // No short format for microseconds, returns in milliseconds
  return `${ms}ms`;
}
...
function fmtLong(ms: number): StringValue {
  ...
  // No long format for microseconds, returns in milliseconds
  return `${ms} ms`;
}
