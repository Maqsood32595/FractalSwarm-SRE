...
function plural(
  ...
  const numUnits = ms / n; // The exact number of units (e.g., -1.5 minutes)
  const roundedValue = Math.round(numUnits); // The rounded value for display (e.g., -1)

  // Determine pluralization based on the absolute value of the exact number of units.
  // If Math.abs(numUnits) is not exactly 1, it should be plural.
  // This handles cases like -1.5 minutes (which is plural) rounding to -1 (which would be singular by standard rounding).
  const isPlural = Math.abs(numUnits) !== 1;
