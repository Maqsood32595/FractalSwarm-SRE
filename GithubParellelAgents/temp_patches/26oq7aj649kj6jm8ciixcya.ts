...
const y = d * 365.25;
const mo = y / 12;
const us = 1 / 1000; // Microseconds to milliseconds conversion
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>milliseconds?|msecs?|ms|microseconds?|mics?|mic|us|μs|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  // The original implementation returned `ms` here, but for consistency with new unit it's better to explicitly return in 'ms' unit.
  // For very small values, it should perhaps attempt to go down to microseconds.
  // However, the issue explicitly mentions parsing, not formatting microseconds.
  // So, keeping the current formatting for `ms` as-is, which handles values less than a second.
  return `${ms}ms`;
}
...
function fmtLong(ms: number): StringValue {
  ...
  // Similar to fmtShort, keep 'ms' for values less than a second.
  return `${ms} ms`;
}
