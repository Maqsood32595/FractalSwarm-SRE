...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>microseconds?|microsecond?|us|μs|milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  // No short format for microseconds as it's typically an internal conversion
  return `${ms}ms`;
}
...
function fmtLong(ms: number): StringValue {
  ...
  // For values less than a second, we can consider showing milliseconds or even microseconds if needed.
  // Given the current formatting logic, it falls to milliseconds.
  // The issue is about parsing microseconds *to* milliseconds, not formatting milliseconds *as* microseconds.
  return `${ms} ms`;
}
