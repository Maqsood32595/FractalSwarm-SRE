...
const y = d * 365.25;
const mo = y / 12;
const µs = 1 / 1000; // 1 microsecond = 0.001 milliseconds
...
type Microseconds = 'microseconds' | 'microsecond' | 'us' | 'µs';
type Unit =
  | Years
  | Months
  | Weeks
...
export function parse(str: string): number {
  ...
  const match =
    /^(?<value>-?\d*\.?\d+) *(?<unit>milliseconds?|msecs?|ms|microseconds?|microsecond?|us|µs|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|weeks?|w|months?|mo|years?|yrs?|y)?$/i.exec(
      str,
    );
...
function fmtShort(ms: number): StringValue {
  ...
  // No short format for microseconds, return milliseconds for now.
  return `${ms}ms`;
}
...
function fmtLong(ms: number): StringValue {
  ...
  // No long format for microseconds, return milliseconds for now.
  return `${ms} ms`;
}
