...
var chunkULG4VSW4_cjs = require('./chunk-ULG4VSW4.cjs');
...
chunkYIPJLB6J_cjs.createProviderDefinedToolFactoryWithOutputSchema({
...
var fileSearch = chunkYIPJLB6J_cjs.createProviderDefinedToolFactoryWithOutputSchema({
...
function createAzure(options = {}) {
  ...
  const getHeaders = () => {
    const baseHeaders = {
      "api-key": chunkYIPJLB6J_cjs.loadApiKey({
    ...
    return chunkYIPJLB6J_cjs.withUserAgentSuffix(baseHeaders, `ai-sdk/azure/${VERSION}`);
  };
  const getResourceName = () => chunkYIPJLB6J_cjs.loadSetting({
...
var AzureOpenAIGateway = class extends chunkYIPJLB6J_cjs.MastraModelGateway {
  constructor(config) {
    super();
    this.config = config;
    this.validateConfig();
  ...
  async resolveLanguageModel({
    ...
    const azureConfig = {
      ...
      headers: { "User-Agent": chunkYIPJLB6J_cjs.MASTRA_USER_AGENT, ...headers },
    ...
    if (useResponsesAPI) {
      ...
      if (websocketFetch) {
        Object.defineProperty(model, chunkYIPJLB6J_cjs.MASTRA_GATEWAY_STREAM_TRANSPORT, {
          configurable: true,
          value: {
...
var GatewayManager = class _GatewayManager {
  ...
  constructor(gateways = []) {
    this.gateways = gateways.filter(chunkAWV2IL46_cjs.shouldEnableGateway);
  }
  /**
  ...
  findGatewayForModel(routerId) {
    return chunkAWV2IL46_cjs.findGatewayForModel(routerId, this.gateways);
  }
  ...
  parseModelId(routerId) {
    ...
    const gatewayId = chunkAWV2IL46_cjs.getGatewayId(gateway);
    const { providerId, modelId } = chunkYYUDVZJC_cjs.parseModelRouterId(routerId, _GatewayManager.getPrefixForId(gatewayId, routerId));
    ...
    for (const gateway of this.gateways) {
      try {
        ...
        for (const [providerId, config] of Object.entries(gatewayProviders)) {
          ...
          registry[key] = { ...config, gateway: chunkAWV2IL46_cjs.getGatewayId(gateway) };
        }
      } catch (error) {
        console.warn(`Failed to load providers from gateway ${chunkAWV2IL46_cjs.getGatewayId(gateway)}:`, error);
      }
    }
    return registry;
  }
...
function getStaticProvidersByGateway(name) {
  return Object.fromEntries(Object.entries(chunkAWV2IL46_cjs.PROVIDER_REGISTRY).filter(([_provider, config]) => config.gateway === name));
}
var defaultGateways = [
  new chunkULG4VSW4_cjs.NetlifyGateway(),
  new chunkAWV2IL46_cjs.MastraGateway(),
  new chunkYYUDVZJC_cjs.ModelsDevGateway(getStaticProvidersByGateway(`models.dev`))
];
...
//# sourceMappingURL=chunk-Z4KN2GTH.cjs.map
//# sourceMappingURL=chunk-Z4KN2GTH.cjs.map