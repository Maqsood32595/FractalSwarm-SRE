...
function splitRuntimeModelString(input) {
  const providers = chunkAWV2IL46_cjs.getRegisteredProviders().sort((a, b) => b.length - a.length);
  ...
  const parsed = chunkAWV2IL46_cjs.parseModelString(input);
...
function isModelAllowed(allowed, candidate) {
  ...
  const activeEntries = allowed.filter((entry) => {
    ...
    return chunkAWV2IL46_cjs.isProviderRegistered(entry.provider);
  });
