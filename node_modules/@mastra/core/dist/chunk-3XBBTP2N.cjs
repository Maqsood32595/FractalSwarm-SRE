...
var CoreToolBuilder = class extends chunkWSD4JNMB_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkHISQ2TWD_cjs.isVercelTool(this.originalTool) && !chunkHISQ2TWD_cjs.isProviderDefinedTool(this.originalTool)) {
        ...
        if (chunkCMKSC37Z_cjs.isZodObject(schema) && isZodV4Schema(schema)) {
          ...
          if (isBackgroundEligible) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
              _background: chunkERDO25O4_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
          ...
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            ...
            const injectedKeys = [];
            if (isBackgroundEligible) {
              properties._background = chunkERDO25O4_cjs.backgroundOverrideJsonSchema;
  ...
  getParameters = () => {
    if (chunkHISQ2TWD_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkHISQ2TWD_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkHISQ2TWD_cjs.isVercelTool(tool)) {
          ...
          if (resumeData) {
            const resumeValidation = chunkHISQ2TWD_cjs.validateToolInput(resumeSchema, resumeData, options.name);
        ...
        if (suspendData) {
          ...
          const suspendValidation = chunkHISQ2TWD_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkHISQ2TWD_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkHISQ2TWD_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      if (fgaProvider) {
        const { getAgentToolFGAResourceId, getMCPToolFGAResourceId, getStandaloneToolFGAResourceId, requireFGA } = await import('./fga-check-NDWK57L6.cjs');
        ...
        await requireFGA({
          ...
          permission: chunk76DMECIQ_cjs.MastraFGAPermissions.TOOLS_EXECUTE,
          requestContext: toolRequestContext,
          actor: execOptions?.actor,
          context: {
            resourceId: options.resourceId
      ...
      try {
        ...
        if (!isResuming) {
          const { data, error } = chunkHISQ2TWD_cjs.validateToolInput(parameters, args, options.name);
  ...
  build() {
    ...
    if (chunkHISQ2TWD_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    const instanceNeedsApprovalFn = chunkHISQ2TWD_cjs.getNeedsApprovalFn(this.originalTool);
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkHISQ2TWD_cjs.Tool) && !chunkHISQ2TWD_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-3XBBTP2N.cjs.map
//# sourceMappingURL=chunk-3XBBTP2N.cjs.map