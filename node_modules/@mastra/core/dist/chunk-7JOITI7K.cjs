...
var chunkXFTKZ4QP_cjs = require('./chunk-XFTKZ4QP.cjs');
...
var chunkKPH2IJW4_cjs = require('./chunk-KPH2IJW4.cjs');
...
var ObservabilityStorage = class _ObservabilityStorage extends chunk2UAP4LDC_cjs.StorageDomain {
  ...
  async getBranch(args) {
    const parsed = chunkYX5KZH6G_cjs.getBranchArgsSchema.parse(args);
    try {
      const skeleton = await this.getStructure({ traceId: parsed.traceId });
      if (!skeleton) return null;
      const branchSpanIds = chunkYX5KZH6G_cjs.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
    ...
    const spans = chunkYX5KZH6G_cjs.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
...
function transformRow(row, tableName, options = {}) {
  ...
  const tableSchema = chunkYX5KZH6G_cjs.TABLE_SCHEMAS[tableName];
  const result = {};
...
function transformScoreRow(row, options = {}) {
  return transformRow(row, chunkYX5KZH6G_cjs.TABLE_SCORERS, options);
}
...
var ObservabilityInMemory = class extends ObservabilityStorage {
  ...
  maybeRegisterBranchCursor(span) {
    if (!chunkYX5KZH6G_cjs.BRANCH_SPAN_TYPE_SET.has(span.spanType)) {
      return;
    }
  ...
  getMatchingRootSpans(args) {
    const { filters, pagination, orderBy } = chunkYX5KZH6G_cjs.listTracesArgsSchema.parse(args);
  ...
  async listTraces(args) {
    const { mode, filters, after, limit } = chunkYX5KZH6G_cjs.listTracesArgsSchema.parse(args);
    if (mode === "delta") {
      ...
      return {
        spans: chunkYX5KZH6G_cjs.toTraceSpans(deltaResponse.rows),
    ...
    return {
      spans: chunkYX5KZH6G_cjs.toTraceSpans(paged),
  ...
  async listBranches(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkYX5KZH6G_cjs.listBranchesArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      ...
      return {
        branches: deltaResponse.rows.map(chunkYX5KZH6G_cjs.toTraceSpan),
        delta: deltaResponse.delta,
        deltaCursor: deltaResponse.deltaCursor
      };
    }
    const allowedSpanTypes = filters?.spanType ? chunkYX5KZH6G_cjs.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : chunkYX5KZH6G_cjs.BRANCH_SPAN_TYPE_SET;
    const matches = [];
    for (const [, traceEntry] of this.db.traces) {
    ...
    return {
      ...
      branches: paged.map(chunkYX5KZH6G_cjs.toTraceSpan),
  ...
  spanMatchesBranchFilters(span, filters) {
    ...
    if (filters.status !== void 0) {
      const spanStatus = chunkYX5KZH6G_cjs.toTraceSpan(span).status;
...
var InMemoryMemory = class extends MemoryStorage {
  ...
  async listMessagesByResourceId({
    ...
    const list = new chunkKPH2IJW4_cjs.MessageList().add(
  ...
  async saveMessages(args) {
    ...
    const list = new chunkKPH2IJW4_cjs.MessageList().add(messages, "memory");
    return { messages: list.get.all.db() };
  }
...
var AgentThreadStreamRuntime = class {
  ...
  async subscribeToThread(agent, options, pubsub) {
    ...
    const onEvent = (event) => {
      ...
      if (data.type === "signal-enqueued") {
        ...
        const queue = signalsByThread.get(key) ?? [];
        queue.push(chunkKPH2IJW4_cjs.createSignal(data.signal));
  ...
  sendMessage(agent, message, target, pubsub) {
    return this.sendSignal(agent, chunkKPH2IJW4_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() }), target, pubsub);
  }
  queueMessage(agent, message, target, pubsub) {
    const state = this.#getState(pubsub);
    const signal = chunkKPH2IJW4_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() });
    let key;
  ...
  async sendStateSignal(agent, stateInput, target, pubsub) {
    ...
    const memoryContext = chunkXFTKZ4QP_cjs.parseMemoryRequestContext(requestContext);
    const memory = await agent.getMemory({ requestContext });
    if (!memory) {
    ...
    const applied = await chunkXFTKZ4QP_cjs.applyStateSignal({
      input: stateInput,
      memory,
      thread,
      resourceId,
  ...
  sendSignal(agent, signalInput, target, pubsub) {
    const state = this.#getState(pubsub);
    let signal = chunkKPH2IJW4_cjs.createSignal({ ...signalInput, acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    let runId = target.runId;
    ...
    const resourceId = target.resourceId ?? activeRecord?.resourceId;
    const threadId = target.threadId ?? activeRecord?.threadId;
    signal = chunkKPH2IJW4_cjs.resolveDeliveryAttributes(
      signal,
...
function createNotificationSignal(notification) {
  return chunkKPH2IJW4_cjs.createSignal({
    type: "notification",
...
var InMemoryStore = class extends chunk2UAP4LDC_cjs.MastraCompositeStore {
  ...
  constructor({ id = "in-memory" } = {}) {
    ...
    this.#db = new chunkTSFPXAIE_cjs.InMemoryDB();
    this.stores = {
      ...
      agents: new chunkTSFPXAIE_cjs.InMemoryAgentsStorage({ db: this.#db }),
      ...
      promptBlocks: new chunkK6M4KRDO_cjs.InMemoryPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkK3N46FKP_cjs.InMemoryScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkRLF3WPN2_cjs.InMemoryMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunk3TUWECFC_cjs.InMemoryMCPServersStorage({ db: this.#db }),
      workspaces: new chunkRNYKMXF6_cjs.InMemoryWorkspacesStorage({ db: this.#db }),
...
var FilesystemStore = class extends chunk2UAP4LDC_cjs.MastraCompositeStore {
  ...
  constructor(config = {}) {
    ...
    this.stores = {
      agents: new chunkTSFPXAIE_cjs.FilesystemAgentsStorage({ db: this.#db }),
      promptBlocks: new chunkK6M4KRDO_cjs.FilesystemPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkK3N46FKP_cjs.FilesystemScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkRLF3WPN2_cjs.FilesystemMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunk3TUWECFC_cjs.FilesystemMCPServersStorage({ db: this.#db }),
      workspaces: new chunkRNYKMXF6_cjs.FilesystemWorkspacesStorage({ db: this.#db }),
...
var StoreOperationsInMemory = class extends StoreOperations {
  ...
  async insert({ tableName, record }) {
    ...
    let key = record.id;
    if (tableName === chunkYX5KZH6G_cjs.TABLE_NOTIFICATIONS && record.threadId && record.id) {
      key = `${record.threadId}\0${record.id}`;
    } else if ([chunkYX5KZH6G_cjs.TABLE_WORKFLOW_SNAPSHOT].includes(tableName) && !record.id && record.run_id) {
...
//# sourceMappingURL=chunk-7JOITI7K.cjs.map
//# sourceMappingURL=chunk-7JOITI7K.cjs.map