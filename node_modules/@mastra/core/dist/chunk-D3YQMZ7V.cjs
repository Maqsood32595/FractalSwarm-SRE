    ...
    outputStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          chunkYK5LWDCV_cjs.attachModelStreamTransport(outputStream, chunkYK5LWDCV_cjs.readModelStreamTransport(stream));
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkKPH2IJW4_cjs.coreContentToString(lastResponseMessage.content) : "";
...
function createProcessorSendSignal(args) {
  return async (signalInput) => {
    const signal = chunkKPH2IJW4_cjs.createSignal(signalInput);
...
function areProcessorMessageArraysEqual(before, after) {
  ...
  return before.length === after.length && before.every((message, index) => chunkKPH2IJW4_cjs.messagesAreEqual(message, after[index]));
}
...
var ProcessorRunner = class _ProcessorRunner {
  ...
  async runComputeStateSignal({
    ...
    const trackingById = chunkXFTKZ4QP_cjs.getStateSignalsMetadata(thread.metadata);
    const tracking = trackingById[stateId];
    const { activeStateSignals, contextWindow, lastSnapshot, deltasSinceSnapshot } = await chunkXFTKZ4QP_cjs.resolveStateSignalHistory({
      messageList,
    ...
    const result = await computeStateSignal({
      ...
      sendStateSignal: async (stateSignal) => {
        const sendResult = await chunkXFTKZ4QP_cjs.applyStateSignal({
          input: stateSignal,
          memory: resolvedMemory,
          thread,
          resourceId: resolvedResourceId,
    ...
    if (!result) return;
    await chunkXFTKZ4QP_cjs.applyStateSignal({
      input: result,
      memory: resolvedMemory,
      thread,
      resourceId: resolvedResourceId,
  ...
  async runOutputProcessors(messageList, observabilityContext, requestContext, retryCount = 0, writer, result) {
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      try {
        ...
        if (processResult instanceof chunkKPH2IJW4_cjs.MessageList) {
  ...
  async runInputProcessors(messageList, observabilityContext, requestContext, retryCount = 0) {
    for (const [index, processorOrWorkflow] of this.inputProcessors.entries()) {
      ...
      try {
        ...
        let mutations;
        if (result instanceof chunkKPH2IJW4_cjs.MessageList) {
      ...
      try {
        ...
        const processMethodArgs = {
          ...
          sendStateSignal: async (stateSignal) => {
            ...
            const result2 = await chunkXFTKZ4QP_cjs.applyStateSignal({
              input: stateSignal,
              memory: resolvedMemory,
              thread,
              resourceId: resolvedResourceId,
  ...
  static async validateAndFormatProcessInputStepResult(result, {
    ...
    if (result instanceof chunkKPH2IJW4_cjs.MessageList) {
      ...
      if (result.model) {
        const resolvedModel = await chunkYK5LWDCV_cjs.resolveModelConfig(result.model);
...
//# sourceMappingURL=chunk-D3YQMZ7V.cjs.map
//# sourceMappingURL=chunk-D3YQMZ7V.cjs.map