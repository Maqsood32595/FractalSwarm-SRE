...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunkKPH2IJW4_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunkKPH2IJW4_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
var A2AAgent = class {
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunkKPH2IJW4_cjs.MessageList({ threadId, resourceId });
