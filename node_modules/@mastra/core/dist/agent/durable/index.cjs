...
var chunkYK5LWDCV_cjs = require('../../chunk-YK5LWDCV.cjs');
var chunkMB3TIFKT_cjs = require('../../chunk-MB3TIFKT.cjs');
var chunk6QR4RQID_cjs = require('../../chunk-6QR4RQID.cjs');
var chunk3XBBTP2N_cjs = require('../../chunk-3XBBTP2N.cjs');
var chunkHISQ2TWD_cjs = require('../../chunk-HISQ2TWD.cjs');
var chunkFHPG32XN_cjs = require('../../chunk-FHPG32XN.cjs');
var chunkLP4WZA6D_cjs = require('../../chunk-LP4WZA6D.cjs');
var chunkERDO25O4_cjs = require('../../chunk-ERDO25O4.cjs');
...
async function runWithIdleWrapper(agent, streamOptions, deps, firstTurn) {
  ...
  const mergedOptions = chunk3XBBTP2N_cjs.deepMerge(
    defaultOptions,
    restStreamOptions ?? {}
  );
...
async function prepareForDurableExecution(options) {
  ...
  const execOptions = chunk3XBBTP2N_cjs.deepMerge(
    defaultOptions ?? {},
    rawExecOptions ?? {}
  );
  ...
  const messageList = new chunkKPH2IJW4_cjs.MessageList({
    threadId,
    resourceId
  });
  ...
  if (inputProcessors.length > 0) {
    try {
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-RF6D3R7A.cjs');
  ...
  const saveQueueManager = memory ? new chunkK5JBD726_cjs.SaveQueueManager({
    logger,
    memory
  }) : void 0;
  ...
  const toolPayloadTransform = chunkKPH2IJW4_cjs.normalizeToolPayloadTransformPolicy(execOptions?.transform) ?? typedAgent.getToolPayloadTransform?.() ?? chunkKPH2IJW4_cjs.normalizeToolPayloadTransformPolicy(
...
function createDurableAgentStream(options) {
    ...
    try {
      switch (streamEvent.type) {
        case AgentStreamEventTypes.CHUNK: {
          ...
          chunkD3YQMZ7V_cjs.safeEnqueue(controller, chunk);
          await onChunk?.(chunk);
          break;
        }
        ...
        case AgentStreamEventTypes.FINISH: {
          ...
          chunkD3YQMZ7V_cjs.safeEnqueue(controller, finishChunk);
          chunkD3YQMZ7V_cjs.safeClose(controller);
          try {
            await onFinish?.(data);
          } catch (callbackError) {
  ...
  const output = new chunkD3YQMZ7V_cjs.MastraModelOutput({
    model,
    stream,
    messageList,
    messageId,
...
function createDurableBackgroundTaskCheckStep() {
  return chunkK5JBD726_cjs.createStep({
    id: BG_CHECK_STEP_ID,
...
async function applyToolPayloadTransformToChunk(chunk, opts) {
  ...
  if (chunk.type === "tool-call") {
    transform = await chunkKPH2IJW4_cjs.transformToolPayloadForTargets(
      {
    ...
    transformedChunk = chunkKPH2IJW4_cjs.withToolPayloadTransformMetadata(
      transformedChunk,
      await chunkKPH2IJW4_cjs.transformToolPayloadForTargets(
        {
          phase: "input-available",
          toolName,
          toolCallId,
  ...
  return chunkKPH2IJW4_cjs.withToolPayloadTransformMetadata(transformedChunk, transform);
}
...
async function resolveRuntimeDependencies(options) {
  ...
  if (memory) {
    saveQueueManager = new chunkK5JBD726_cjs.SaveQueueManager({
...
async function toolRequiresApproval(tool, globalRequireApproval, args, approvalContext) {
  ...
  const needsApprovalFn = chunkHISQ2TWD_cjs.getNeedsApprovalFn(tool);
  if (needsApprovalFn) {
    try {
...
async function resolveModelFromConfig(config, mastra) {
  ...
  if (typeof modelConfigString === "string") {
    return await chunkYK5LWDCV_cjs.resolveModelConfig(modelConfigString, requestContext, mastra);
  }
  return await chunkYK5LWDCV_cjs.resolveModelConfig(
    modelConfigString,
    requestContext,
    mastra
  );
      ...
      for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (!chunkD3YQMZ7V_cjs.isSupportedLanguageModel(model)) {
            ...
            let currentProviderOptions = chunkK5JBD726_cjs.mergeProviderOptions(
            ...
            const stepInputProcessors = registryEntry?.prepareStep ? [...baseInputProcessors, new chunkK5JBD726_cjs.PrepareStepProcessor({ prepareStep: registryEntry.prepareStep })] : baseInputProcessors;
            if (stepInputProcessors.length) {
              ...
              const runner = new chunkD3YQMZ7V_cjs.ProcessorRunner({
            ...
            const modelResult = chunkK5JBD726_cjs.execute({
              runId,
              model: currentModel,
              providerOptions: currentProviderOptions,
              inputMessages,
            ...
            const outputStream = new chunkD3YQMZ7V_cjs.MastraModelOutput({
              model: {
            ...
            if (registryEntry?.errorProcessors?.length) {
              try {
                ...
                const currentMessageList = new chunkKPH2IJW4_cjs.MessageList();
  ...
  return chunkK5JBD726_cjs.createStep({
    ...
    execute: async (params) => {
      ...
      if (bgManager && !bgConfig?.disabled && typeof cleanedArgs === "object" && cleanedArgs !== null) {
        const bgResolved = chunkERDO25O4_cjs.resolveBackgroundConfig({
          llmBgOverrides,
          toolName,
        ...
        if (bgResolved.runInBackground) {
          try {
            const bgTask = chunkERDO25O4_cjs.createBackgroundTask(bgManager, {
              toolName,
              toolCallId,
              args: cleanedArgs,
  ...
  return chunkK5JBD726_cjs.createStep({
    ...
    execute: async (params) => {
      ...
      const ctx = {
        ...
        threadId: initData.state?.threadId,
        resourceId: initData.state?.resourceId,
        customContext: void 0
      };
      let result;
      try {
        result = await chunkK5JBD726_cjs.runStreamCompletionScorers(isTaskComplete.scorers, ctx, {
      ...
      const feedback = chunkK5JBD726_cjs.formatStreamCompletionFeedback(result, maxIterationReached);
      messageList.add(
        {
...
function createDurableAgenticWorkflow(options) {
  ...
  const singleIterationWorkflow = chunkK5JBD726_cjs.createWorkflow({
  ...
  return chunkK5JBD726_cjs.createWorkflow({
    ...
    async (params) => {
      ...
      if (registryEntry?.outputProcessors?.length) {
        try {
          ...
          const outputMessageList = new chunkKPH2IJW4_cjs.MessageList();
      ...
      if (registryEntry?.saveQueueManager && registryEntry.memory && durableState?.threadId && durableState?.resourceId && !durableState.observationalMemory) {
        try {
          const memoryMessageList = new chunkKPH2IJW4_cjs.MessageList();
    ...
    async (params) => {
      ...
      const logger = mastra?.getLogger?.();
      const inputMessageList = new chunkKPH2IJW4_cjs.MessageList();
...
var DurableAgent = class extends chunkK5JBD726_cjs.Agent {
  /** The wrapped agent */
  #wrappedAgent;
  ...
  constructor(config) {
    ...
    this.#innerPubsub = pubsub ?? new chunkMB3TIFKT_cjs.EventEmitterPubSub();
   ...
   */
    ...
    if (this.#cacheConfig === false) {
    ...
    } else if (this.#innerPubsub instanceof chunkPULYUCGC_cjs.CachingPubSub) {
      this.#cachingPubsub = this.#innerPubsub;
      ...
      this.#cachingPubsub = new chunkPULYUCGC_cjs.CachingPubSub(this.#innerPubsub, resolvedCache);
    }
  }
  // ===========================================================================
