...
function getDisplayTransform(metadata, phase, fallback) {
  const transform = chunkKPH2IJW4_cjs.getTransformedToolPayload(metadata, "display", phase);
  return chunkKPH2IJW4_cjs.hasTransformedToolPayload(transform) ? transform.transformed : fallback;
}
...
function toUserSignalMessage(payload) {
  ...
  const signal = chunkKPH2IJW4_cjs.createSignal({
    id,
    type: "user",
    tagName: "user",
...
function describeServerSideFallback(providerMetadata) {
  const fallback = chunkK5JBD726_cjs.getServerSideFallbackInfo(providerMetadata);
  if (!fallback) {
    return void 0;
  }
  ...
  async processStreamChunk(state, chunk, requestContext) {
    ...
    switch (chunk.type) {
      ...
      case "tool-call-delta": {
        ...
        const transform = chunkKPH2IJW4_cjs.getTransformedToolPayload(chunk.metadata, "display", "input-delta");
        if (!transform?.suppress) {
          this.#session.emit({
            type: "tool_input_delta",
            toolCallId,
            argsTextDelta: chunkKPH2IJW4_cjs.hasTransformedToolPayload(transform) ? transform.transformed : argsTextDelta,
            toolName
          });
        }
        break;
      ...
      case "tool-call-approval": {
        ...
        const approvalTransform = chunkKPH2IJW4_cjs.getTransformedToolPayload(chunk.metadata, "display", "approval");
        const toolArgs = chunkKPH2IJW4_cjs.hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", chunk.payload.args);
...
var SessionOMRole = class {
  ...
  resolvedModel() {
    ...
    return new chunkYK5LWDCV_cjs.ModelRouterLanguageModel(modelId, this.#gateways);
  }
  ...
  apply(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : chunk3XBBTP2N_cjs.safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
...
var Session = class {
  ...
  constructor({
    ...
    if (!workspace || !(workspace instanceof chunkK5JBD726_cjs.Workspace)) {
...
function createSubagentTool(opts) {
  ...
  return chunkHISQ2TWD_cjs.createTool({
    id: "subagent",
...
${FORKED_SUBAGENT_TASK_NOTICE}`;
        ...
        subagentToRun = new chunkK5JBD726_cjs.Agent({
        ...
        const allWorkspaceToolNames = workspace && allowedWs ? new Set(
          Object.keys(
            await chunkK5JBD726_cjs.createWorkspaceTools(workspace, {
...
function patchTaskToolForFork(tool, toolId) {
  const stubExecute = async () => {
    ...
    if (toolId === "task_check") {
      const taskCheck = chunkK5JBD726_cjs.summarizeTaskCheck([]);
      return {
        ...baseResult,
...
var AgentController = class {
  ...
  constructor(config) {
    ...
    this.#gatewayManager = new chunkZ4KN2GTH_cjs.GatewayManager([...config.gateways ?? [], ...chunkZ4KN2GTH_cjs.defaultGateways]);
  ...
  async #createSessionForResource(ownerId, id, effectiveResourceId, tags, overrides) {
    ...
    if (workspaceToConnect && workspaceToConnect instanceof chunkK5JBD726_cjs.Workspace) {
      try {
  ...
  async runInit() {
    if (this.config.storage && !this.#externalMastra) {
      ...
      this.#internalMastra = new chunkK5JBD726_cjs.Mastra({
        logger: false,
    ...
    if (this.config.workspace && !this.workspaceInitialized && typeof this.workspace !== "function") {
      try {
        if (!this.workspace) {
          this.workspace = new chunkK5JBD726_cjs.Workspace(this.config.workspace);
        }
  ...
  getAgentForMode(mode) {
    ...
    if (!this.#legacyAgentMode[mode.id]) {
      ...
      this.#legacyAgentMode[mode.id] = new chunkK5JBD726_cjs.Agent({
        id: `${this.id}-agent`,
  ...
  convertToControllerMessage(msg) {
    ...
    if (msg.role === "signal") {
      const signal = chunkKPH2IJW4_cjs.mastraDBMessageToSignal(msg);
      if (signal.type === "user") {
  ...
  async buildToolsets(session, requestContext) {
    const builtInTools = {
      ask_user: chunkK5JBD726_cjs.askUserTool,
      submit_plan: chunkK5JBD726_cjs.submitPlanTool,
      task_write: chunkK5JBD726_cjs.taskWriteTool,
      task_update: chunkK5JBD726_cjs.taskUpdateTool,
      task_complete: chunkK5JBD726_cjs.taskCompleteTool,
      task_check: chunkK5JBD726_cjs.taskCheckTool
    };
...
//# sourceMappingURL=chunk-XWXLR5MP.cjs.map
//# sourceMappingURL=chunk-XWXLR5MP.cjs.map